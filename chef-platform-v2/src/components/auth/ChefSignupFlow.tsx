  ...
  const CardBody = (
      <Card className="w-full max-w-xl bg-card rounded-3xl shadow-2xl border-border overflow-hidden relative">
        ...
        <div className="p-6 space-y-6">
          {step === 1 && (
            <div>
              <div className="text-center mb-6">
                <div className="w-12 h-12 rounded-full bg-pink-500 text-white mx-auto mb-4 flex items-center justify-center">
                  <span className="text-xl">⚕︎</span>
                </div>
                <h2 className="text-2xl font-semibold">Join Table & Plate as a Chef</h2>
          ...
          {step === 2 && (
            <div>
              <h2 className="text-2xl font-semibold text-center mb-2">Verify your number</h2>
              <p className="text-center text-muted-foreground mb-6">We've sent a 6-digit code to {mobile || "your mobile"}</p>
              <div>
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-center text-muted-foreground mb-6">Build your professional chef profile</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <Textarea value={bio} onChange={(e) => setBio(e.target.value)} maxLength={500} placeholder="Tell potential clients about your culinary journey, training, and passion for cooking..." className="rounded-2xl min-h-[120px]" />
          ...
          {step === 4 && (
            <div>
              <h2 className="text-2xl font-semibold text-center mb-2">Showcase your work</h2>
              <p className="text-center text-muted-foreground mb-6">Add a profile photo and gallery images of your dishes</p>
              ...
              <div className="space-y-2 mb-6">
                ...
                <div className="rounded-2xl border border-dashed border-border/60 bg-muted/20 p-4">
                  <div className="aspect-[3/1.3] w-full rounded-xl flex items-center justify-center">
                    {profilePhoto ? (
                      <div className="relative w-full h-full">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={profilePhoto} alt="Profile" className="w-full h-full object-cover rounded-xl" />
          ...
          {step === 5 && (
            <div>
              <h2 className="text-2xl font-semibold text-center mb-2">Your expertise</h2>
              <p className="text-center text-muted-foreground mb-6">Help clients understand your culinary skills</p>

              <div className="space-y-6">
                {/* Specialties */}
                <div>
                  <label className="block text-sm mb-2">Culinary specialties</label>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      ...
                      <button key={sp} onClick={() => toggle(specialties, setSpecialties, sp)} className={`px-3 py-2 rounded-2xl border text-sm ${specialties.includes(sp) ? 'bg-primary text-primary-foreground border-primary' : 'bg-background border-border hover:border-foreground/40'}`}>{sp}</button>
                    ))}
                  </div>
                </div>
        ...
        <div className="p-6 border-t border-border/60 flex items-center justify-between">
          ...
          {step < TOTAL_STEPS ? (
            <Button disabled={!canNext()} className="rounded-2xl bg-gradient-to-br from-pink-500 to-rose-500 text-white hover:from-pink-600 hover:to-rose-600" onClick={() => setStep((s) => Math.min(TOTAL_STEPS, s + 1))}>
              Next <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
          ) : (
            <Button disabled={!canNext()} className="rounded-2xl bg-gradient-to-br from-pink-500 to-rose-500 text-white hover:from-pink-600 hover:to-rose-600">
